...
//Reads and merges the records from the output pipe and the sorted file into the cache file in Asc order.
...
Sorted::Sorted(OrderMaker myOrder, int runLength, ExtendedDBFile* file, ExtendedDBFile* cache, string fpath, string cache_path) {
    m_fileType = sorted;
    m_file = file;
    m_order = myOrder;
...
int Sorted::Create(const char *fpath, fType file_type, void *startup) {
    ...
    run_Length = (*((SortInfo*)startup)).runLength;
    return 1;
}
...
DBFile::DBFile () {
    m_file = new ExtendedDBFile();
    m_cache = new ExtendedDBFile();
    m_instance = NULL;
}
...
Heap::Heap(ExtendedDBFile* file, ExtendedDBFile* cache,  string fpath, string cache_path) {
    m_fileType = heap;
    m_file = file;
    m_cache = cache;
    m_file_path = fpath;
